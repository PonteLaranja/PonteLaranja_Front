...
import { listar_tipoItem } from "@/pages/api/tipoItem";
import { listar_tipoMedida } from "@/pages/api/tipoMedida";
import {
  listarLogs,
  listarLogs_por_Unidade,
  Transferencia,
} from "@/pages/api/logHistorico";
import { listarItens } from "@/pages/api/item";
import { listarUsuario, listUsuario } from "@/pages/api/usuario";
...
export const Banner = () => {
  ...
  const itensFiltrados = logV2.filter((item) => {
    ...
    const matchTipo =
      ...
      item.tipoItemID === filtroTipoItem;

    const matchUnidade =
      filtroUnidade === "" ||
      filtroUnidade === "todos" ||
      item.unidadeDestinoID === filtroUnidade;

    const matchData =
      filtroData === "" ||
      filtroData === "todos" ||
      item.dataChegada?.split("T")[0] === filtroData;
